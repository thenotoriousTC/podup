  ...
  return (
    <View className="bg-white rounded-2xl p-6 shadow-lg items-center mb-8">
      <View className={`w-32 h-32 rounded-full justify-center items-center mb-6 ${
        isRecording ? 'bg-red-50 border-4 border-red-500' : 'bg-white border-4 border-[#FD842B]'
      }`}>
        <Ionicons 
          name={isRecording ? "mic" : "mic"} 
          size={64} 
          color={isRecording ? "#FF3B30" : "#FD842B"} 
        />
      </View>
      
      <StyledText className="text-2xl font-semibold text-[#FD842B] mb-3 text-center">
        {isRecording ? " جاري التسجيل" : "جاهز للتسجيل"}
      </StyledText>
      {isRecording && (
        <StyledText className="text-2xl font-semibold text-[#FD842B] mb-3">
      ...
      <TouchableOpacity 
        className={`px-8 py-4 rounded-full mt-3 ${
          isRecording ? 'bg-red-500' : 'bg-[#FD842B]'
